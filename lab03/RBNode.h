...
struct RBNode {
	...
	RBNode(int k, RBNode *l = Nil, RBNode *r = Nil, RBNode *parent = Nil, color c = RBNode::BLACK) : key(k), p(parent),
		left(l), right(r), col(c) {
	}
...
struct RBTree {
	...
	int maxBlackKey(RBNode *x) {
		...
		int best = (leftMax > rightMax) ? leftMax : rightMax;
		if (x->col == RBNode::BLACK) {
			if (x->key > best) best = x->key;
		}
		return best;
	}
